...
def approve_renewal_mis(record_name):
    ...
    frappe.db.commit()
    frappe.msgprint(f"Renewal approved. Insurance Record updated.", indicator="green", alert=True)
...
def reject_renewal_mis(record_name, reason=None):
    ...
    frappe.db.commit()
    frappe.msgprint("Renewal rejected.", indicator="red", alert=True)
